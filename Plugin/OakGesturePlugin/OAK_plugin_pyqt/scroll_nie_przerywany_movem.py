""" INFO:
Ten skrypt to kopia `oak_plugin`, ale gest SCROLL (SLAB: WHEEL UP / DOWN)
ma ustalone `duration = 2s`, więc scrollowanie trwa 2 sekundy od momentu rozpoznania gestu.
Domyślnie gest nie jest przerywany przez inne gesty, np. `MOV` (zoom).

Natomiast ja uznałem, że lepiej jest przerywać scrollowanie (WHEEL)
w momencie wykonania innego gestu — np. `MOV` — żeby użytkownik miał większą kontrolę
nad przewijaniem kółkiem myszy i mógł je w dowolnej chwili zatrzymać.

Dlatego w tej wersji skryptu scroll NIE jest automatycznie przerywany, gdy wykryty zostanie inny gest.
"""
...
#                 scroll_overlay_message = ""
#
#             cv2.imshow("PyMOL Gesture Mouse Mode", frame)
